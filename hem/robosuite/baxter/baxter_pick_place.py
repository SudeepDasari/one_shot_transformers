...
from hem.robosuite.baxter.baxter_robot import BaxterRobot


class BaxterPickPlace(BaxterEnv):
    def __init__(
        self,
        ...
        self.finger_names = self.gripper_right.contact_geoms()
    ...
    def _pre_action(self, action):
        padded_action = np.concatenate((action[:7], np.zeros(7)))
        if self.has_gripper_right:
            padded_action = np.concatenate((padded_action, action[-1:]))
        return super()._pre_action(padded_action)
...
class BaxterPickPlaceBread(BaxterPickPlace):
    """
...
class BaxterPickPlaceCereal(BaxterPickPlace):
    """
...
class BaxterPickPlaceCan(BaxterPickPlace):
    """
